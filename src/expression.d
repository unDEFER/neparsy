...
class Expression
{
    ...
    Position start_comment(out uint[] path)
    {
        ...
        if (path.empty)
        {
            ...
            foreach(i, arg; arguments)
            {
                ...
                if (!spath.empty && (pos.row == 0 || spos.row > 0 && spos < pos))
                {
                    pos = spos;
                    m = cast(uint) (i+1);
                    mpath = spath;
            ...
            {
                path = [m] ~ mpath;
                writefln("START_COMMENT %s: %s, %s", *(get_lexem_by_path(path)), path, pos);
            }
        }
        else
            writefln("START_COMMENT %s: %s, %s", *(get_lexem_by_path(path)), path, pos);
    ...
    Lexem *get_lexem_by_path(uint[] path)
    {
        ...
        foreach (p; path[0..$-1])
        {
            if (p-1 >= t.arguments.length) return null;
            t = t.arguments[p-1];
        }

        writefln("PATH %s AT %s", path, t);
        switch (path[$-1])
        {
            case 0:
                return &t.open_lexem;
    ...
    void merge_inp(Expression inp)
    {
        ...
        if (open_lexem.comments.length != inp.open_lexem.comments.length) return;
        for (int i = 0; i < open_lexem.comments.length; i++)
        {
            ...
            if (l is null || il is null) return;
            icom.text = "***";
            l.comments ~= com;
            il.comments ~= icom;
    ...
    string saveD(ref string resstr, ref Position pos, int tab = 0, Expression[] post = null, string ptype = null)
    {
        ...
        if (!handled)
        {
            ...
            else switch(this.type)
            {
                ...
                case "static-if":
                    if (type == "static-if")
                        savePrint(resstr, pos, "static ", type_lexem);
                    ...
                    foreach(i, arg; this.arguments[1..$])
                    {
                        ...
                        {
                            arg.saveD(resstr, pos, tab, null, "if");
                        }
                ...
                default:
                    switch (this.operator)
                    {
                        ...
                        default:
                            ...
                            if (ptype == "foreach" && postop !is null)
                            {
                                postop.saveD(resstr, pos, tab, null, ptype);
                            }
